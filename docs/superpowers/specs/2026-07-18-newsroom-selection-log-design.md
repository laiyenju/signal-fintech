...
```jsonc
{
  ...
  "tw": {
    ...
    "scoredPool": [                          // 新增：B 深度。所有「過資格＋被打分」的候選（含落選）
      {
        "eventKey": "…",
        "source": "TechCrunch Fintech",     // 該候選的「來源 feed 名」＝ raw_items 裡的 source（用於汰換歸因；與 candidate.json 對外標示的原始媒體是兩回事。TLDR 拆出的項目 source 記為 "TLDR Fintech" 等 feed 名）
        "class": "A",
...
- **來源名冊**：從 `scripts/feeds.py`（本次由 `fetch_news.py` 抽出的純資料 `FEEDS`）取得完整資料源清單，才能列出「本輪完全沒更新」的靜默源（windowItems=0）。
- **行為**：
  1. 以 feeds.py 的 FEEDS 為名冊，由 raw_items.json 依 `source`+`scope` 分組算 `windowItems`（未出現者為 0）；由 meta.scoredPool 算各源 `contributed`（decision≠dropped）。
...
| 動作 | 檔案 |
|---|---|
| 新增 | `scripts/feeds.py`（從 fetch_news.py 抽出的純資料 `FEEDS`，作為資料源名冊單一事實來源） |
| 修改 | `scripts/fetch_news.py`（改為 `from feeds import FEEDS`，行為不變） |
