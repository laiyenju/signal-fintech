...
4. **文筆**：paras 恰兩段、沒有「發生什麼／影響／為什麼重要」小標、專有名詞保留原文（必要時括號說明）、繁體中文通順。另依 `排程任務指令.md` 第 6.2.1 節抽查風格（validate.py 已硬擋禁用詞清單與「報導指出」次數，你抓的是清單外的問題，這些記 `minor`）：
   - 漏網的內部術語或「工作日誌」口吻（對讀者解釋選稿流程、規則、資料來源管線）。
   - 免責句或段落開頭句式樣板化（多則新聞長得像同一句的排列組合，例如每則第二段都以同一句式收尾）。
   - 括號英文注釋過度：常用詞注英文、台灣機構注英文名、金額雙寫（如「5,250萬美元（US$52.5M）」）、同一則內同詞重複注。
   - 超過約 80 字的單句，或失控的頓號長列舉。
   - 中英文、中文與數字之間缺半形空格。
